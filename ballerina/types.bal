...
# + interrupt - Set when the agent paused for human input (Human-in-the-Loop). The caller
#   provides the human's response as the next chat message to resume the agent.
...
type ExecutionTrace record {|
    ...
    # Set when the agent paused to request human input during this execution.
    Interrupt? interrupt = ();
|};
